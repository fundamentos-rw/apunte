...
diccionario |D| con $N$ pares clave-valor, la operación |D[k]| es de tiempo
constante.

Dado que las claves pueden ser de cualquier tipo (a diferencia de las listas, en
las que los índices son números enteros entre 0 y $N-1$), el algoritmo
utilizado para almacenar los datos en el diccionario es más sofisticado que el
utilizado para las listas.

Los diccionarios de Python uilizan un algoritmo llamado \emph{hash},
que se basa en realizar un cálculo numérico sobre la clave del elemento (y es
el que impone la restricción de que las claves deben ser inmutables).
\end{sabias_que}
