...
\begin{itemize}
        \item En Windows, presionar las teclas \keys{Windows}~+~\keys{R}, luego
            escribir |cmd| y presionar \keys{Enter}.
        \item En Mac OS, presionar las teclas \keys{\cmdmac}~+~\keys{Espacio},
            luego escribir |terminal|  y presionar \keys{Enter}.
        \item En Linux (Ubuntu), presionar \keys{Ctrl}~+~\keys{Alt}~+~\keys{T}.
\end{itemize}
...
\subsection{El intérprete interactivo de Python}
...
\circled{1} Las líneas que comienzan con \verb|$| indican órdenes que le damos
al sistema operativo (en este caso la orden es |python3|, es decir \emph{abrir
el intérprete de Python}).

\circled{2} Para orientarnos, el intérprete de Python muestra los símbolos |>>>|
...
Vamos a elegir punto flotante cuando nos interese más la magnitud y no tanto la
...
&
Se despliega el texto \enquote{Se calcularán cuadrados de
...
&
Se despliega el texto \enquote{Ingrese un número entero: } en la pantalla y el
...
&
Se despliega el texto \enquote{Ingrese otro número entero:} en la
