...
un elemento de la lista por vez y hacer una llamada recursiva. Olvidémonos por
un momento de la recursividad e imaginemos que \emph{ya} tenemos una función
que sabe sumar los elementos en una lista y que lo hace bien.

Ahora queremos resolver otro problema: Dada la solución al problema de sumar
los elementos de una lista qué pasaría si la lista tuviera un elemento
...
\lstinline!sumar2! ya implementada que nos dice la suma de los elementos de \lstinline!lista!.
La solución es sencilla: La suma de la lista ampliada será la suma de la lista
más la cabeza.
...
def sumar(lista):
   ...
   return sumar2(lista[1:]) + lista[0]
\end{codigo-python-sn}

Podemos ver que si tuviéramos implementada \lstinline!buscar2! entonces
\lstinline!buscar! funcionaría bien. Volvamos ahora a recursividad: Si sabemos
...
funciones invocantes. Por ejemplo, para resolver el resultado de Fibonacci
$F_5$ se utilizaron los resultados computados por $F_4$ y $F_3$. Las funciones
recursivas no recibieron resultados del cómputo de las funciones invocantes.
Esto no siempre es así, en algunos problemas sí hace falta recibirlos.

A veces, si bien no es necesario, puede tener ventajas adicionales propagar la
información hacia abajo. Por ejemplo, podríamos reescribir la función
\lstinline!sumar! de esta forma:
\begin{codigo-python-sn}
def sumar(lista, suma=0):
    ...
    return sumar(lista[1:], suma + lista[0])
\end{codigo-python-sn}
...
principio, le \emph{pasamos} la solución parcial a las llamadas recursivas.
...
ninguna operación adicional sobre esta llamada) se conoce como
...
def sumar(lista):
    ...
    while True:
        ...
        lista, suma = lista[1:], suma + lista[0]
\end{codigo-python-sn}
tan solo reemplazando la recursión por una iteración y actualizando las
variables según los parámetros de la llamada recursiva. Algunos lenguajes
de programación (no Python) identifican la recursividad de cola y directamente
ejecutan la versión iterativa, esto se conoce como \emph{optimización de
recursividad de cola}.

\section{Modificación de la interfaz}

En el ejemplo de \lstinline!sumar! implementada con recursividad de cola nos
vimos obligados a modificar la interfaz de la función agregando la variable
\lstinline!suma! que no formaba parte del problema inicial. Pudimos hacerlo
elegantemente utilizando un valor por omisión, pero la interfaz de todos modos
quedó confusa. Hay veces en la que no podemos salvar un cambio en la interfaz.
...
complicaría innecesariamente el código. Es preferible modificar la interfaz
del problema.

Implementemos el problema resolviendo primero la llamada recursiva y luego
ensamblando:
\begin{codigo-python-sn}
def _promediar(lista):
    ...
    return suma + lista[0], cantidad + 1
\end{codigo-python-sn}
...
natural del problema que nos pidieron. Se devuelven dos cosas en vez de una.
...
\end{codigo-python-sn}
Si bien la función visible \lstinline!promediar! no es recursiva, sí lo
es la función \lstinline!_promediar! que es la que realiza el trabajo, por
lo que el conjunto se considera recursivo.

Además de para \emph{acomodar} los parámetros a los necesarios para realizar
la primera llamada a la función recursiva, las funciones wrapper se suelen utilizar para
simplificar el código de las funciones recursivas. Por ejemplo, si quisiéramos hacer comprobaciones no
querríamos que las mismas se reiteraran en cada iteración recursiva porque
consumirían recursos innecesarios. Entonces las resolvemos antes de empezar la recursión.

Por ejemplo, en el capítulo~\ref{recursion_potencia} implementamos la potencia
