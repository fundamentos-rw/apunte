...
Pero un \enquote{truco} que permiten algunos lenguajes, entre ellos Python, es la
...
La \enquote{casi}--inversa de \lstinline!split! es una función \lstinline!join!
...
matrices (o, en general, arreglos de dos o más dimensiones).
...
listas. Hay muchas alternativas para representar la matriz, pero hay dos que son
las más frecuentemente utilizadas:

\begin{enumerate}

\item La forma más usual de representar matrices es mediante una \emph{lista de filas}:

\begin{codigo-python-sn}
>>> m = [[1, 2, 3], [4, 5, 6]]
\end{codigo-python-sn}

De esta forma, las dimensiones de la matriz (cantidad de filas y columnas) están dadas por
las longitudes de las listas. Y si quisiéramos acceder a un elemento particular sabiendo
su fila y columna, usaremos la notación |m[fila][columna]|.
...
\item Una forma menos frecuente (pero igualmente válida) es representar la
matriz como una \emph{lista de columnas}:

\begin{codigo-python-sn}
>>> m = [[1, 4], [2, 5], [3, 6]]
\end{codigo-python-sn}

Todas las operaciones resultan muy similares, teniendo cuidado en invertir el
...
Ya conocemos dos opciones para representar matrices: lista de filas y lista de columnas.
¿Alguna de ellas es mejor que la otra? ¿Cómo decidimos cuál utilizar?

Tal vez el factor más importante a considerar es en qué orden vamos a
querer \emph{recorrer} la matriz al manipularla. Hay dos maneras obvias de recorrer
los elementos de una matriz: \emph{orden principal de fila} y \emph{orden principal de columna},
mostrados en la Figura \ref{orden-matriz}.
...
\begin{tikzpicture}
  ...
  \node[node distance=0pt,anchor=south,above=of f.north] {orden principal de fila};
  \matrix[anchor=west,right=4cm of f.east] (c)
  {
    \node (ca11) {$a_{11}$}; & \node (ca12) {$a_{12}$}; & \node (ca13) {$a_{13}$}; \\
    \node (ca21) {$a_{21}$}; & \node (ca22) {$a_{22}$}; & \node (ca23) {$a_{23}$}; \\
    \node (ca31) {$a_{31}$}; & \node (ca32) {$a_{32}$}; & \node (ca33) {$a_{33}$}; \\
  };
  \node[node distance=0pt,anchor=south,above=of c.north] {orden principal de columna};
  \begin{scope}[transparency group, opacity=0.3]
      \draw[flecha,line width=0.5ex] (fa11.center)--(fa13.center)--(fa21.center)--(fa23.center)--(fa31.center)--(fa33.center);
      \draw[flecha,line width=0.5ex] (ca11.center)--(ca31.center)--(ca12.center)--(ca32.center)--(ca13.center)--(ca33.center);
  \end{scope}
\end{tikzpicture}
\caption{Dos maneras de recorrer los elementos de una matriz.}
\label{orden-matriz}
\end{figure}

Si normalmente vamos a querer recorrer la matriz en orden principal de fila,
entonces la representación de lista de filas es más adecuada. Esto es lo más
usual.

En cambio, si frecuentemente vamos a recorrer en orden principal de columna,
entonces elegiremos lista de columnas.
...
tipo |function| (la función llamada \enquote{exclamar}). Al hacer |f = exclamar|,
...
\begin{itemize}
\item |lambda x: x * 2| es \enquote{una función que dado un número $x$ devuelve $2x$}.
\item |lambda x, y: x + y| es \enquote{una función que dados $x$ e $y$ devuelve la suma de ambos}.
\end{itemize}
