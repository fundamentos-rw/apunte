...
\enquote{Numero positivo}, si el número es igual a 0 un mensaje \enquote{Igual a
0}, y si el número es negativo escribir un mensaje \enquote{Numero
negativo}.
\end{quote}
...
\begin{tikzpicture}[
    ...
    \node [below=of exp,bloque] (cuerpo) {<cuerpo>};
    \node [right=2cm of exp,end] (end) {};
    \path [flecha] (init) -- (exp);
    \path [flecha] (exp) -- node[right,near start] {True} (cuerpo);
    \path [flecha] (cuerpo) -- ++(-2,0) -| ++(0,2);
    \path [flecha] (exp) -- node[above,near start] {False} (end);
\end{tikzpicture}
...
Definimos una variable \lstinline!hay_mas_datos!, que valdrá \enquote{Si} mientras
haya datos.

Se le debe preguntar al usuario, después de cada cálculo, si hay o no más datos.
Cuando el usuario deje de responder \enquote{Si}, dejaremos de ejecutar el cuerpo del
ciclo.
...
tenía algún dato para calcular, y darle el valor inicial \enquote{Si} a
...
instrucción |break|, la ejecución \enquote{sale} del ciclo inmediatamente.
...
instrucción |continue|, la ejecución \enquote{saltea} la iteración actual del ciclo y
pasa a la siguiente.
...
\item Un ciclo \enquote{infinito} que se corta.
\end{enumerate}
}
...
\enquote{Saltea} la ejecución del ciclo a la siguiente iteración. Puede utilizarse
